...
class SunTest(unittest.TestCase):
    ...
    def test_gmst0(self):
        self.assertAlmostEqual(
            self.sun.gmst0(self.days), 100.63516802400045, delta=self.tol
        )
    ...
    def test_altitude_times_arbitrary_altitude_no_limb_correction(self):
        (sunrise, sunset) = self.sun.altitude_times(
            self.timestamp, self.longitude, self.latitude, self.arb_alt, False
        )
    ...
    def test_altitude_times_arbitrary_altitude_with_limb_correction(self):
        (sunrise, sunset) = self.sun.altitude_times(
            self.timestamp, self.longitude, self.latitude, self.arb_alt, True
        )
    ...
    def test_rise_set(self):
        (sunrise, sunset) = self.sun.rise_set(
            self.timestamp, self.longitude, self.latitude
        )
    ...
    def test_civil_twilight(self):
        (sunrise, sunset) = self.sun.civil_twilight(
            self.timestamp, self.longitude, self.latitude
        )
    ...
    def test_nautical_twilight(self):
        (sunrise, sunset) = self.sun.nautical_twilight(
            self.timestamp, self.longitude, self.latitude
        )
    ...
    def test_astronomical_twilight(self):
        (sunrise, sunset) = self.sun.astronomical_twilight(
            self.timestamp, self.longitude, self.latitude
        )
